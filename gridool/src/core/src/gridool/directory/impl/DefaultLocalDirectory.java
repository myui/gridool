...
import gridool.directory.btree.BIndexFile;
import gridool.directory.btree.BIndexMultiValueFile;
...
import gridool.directory.btree.Paged;
import gridool.directory.btree.Value;
import gridool.directory.btree.BasicIndexQuery.IndexConditionEQ;
import gridool.directory.btree.BasicIndexQuery.IndexConditionSW;
...
public final class DefaultLocalDirectory extends AbstractLocalDirectory {
    ...
    private BIndexFile createIndex(final String name, final boolean create) throws IndexException {
        ...
        if(DELETE_IDX_ON_EXIT) {
            if(idxFile.exists()) {
                if(!idxFile.delete()) {
                    throw new IndexException("Could not prepared a file: " + idxFile.getAbsolutePath());
                }
            }
        }
        if(create) {

...
public final class DBInsertOperation extends DBOperation {
	...
	public DBInsertOperation(String driverClassName, String connectUrl,
			@CheckForNull String tableName, @CheckForNull String[] fieldNames,
			@CheckForNull DBRecord[] records) {
		this(driverClassName, connectUrl, null, tableName, fieldNames, records);
	}

	public DBInsertOperation(String driverClassName, String connectUrl,
			@Nullable String createTableDDL, @CheckForNull String tableName,
			@CheckForNull String[] fieldNames, @CheckForNull DBRecord[] records) {
	...
	private static void checkParameters(String tableName, String[] fieldNames,
			DBRecord[] records) {
		if (tableName == null) {
		...
		if (records == null || records.length == 0) {
			throw new IllegalArgumentException(
					"No insert record is not allowed");
		}
	}
	...
	public Serializable execute() throws SQLException {
		...
		if (createTableDDL != null) {
			try {
				...
				LOG.warn("Table already exists. Try to truncate " + tableName,
						e);
	...
	private static void executeDDL(@Nonnull final Connection conn,
	...
	private static void truncateTable(@Nonnull final Connection conn,
	...
	private static void executeInsertQuery(@Nonnull final Connection conn,
			@Nonnull final String sql, @Nonnull final DBRecord[] records)
			throws SQLException {
	...
	private static String constructQuery(@Nonnull final String tableName,
			@Nullable final String[] fieldNames,
			@Nonnull final DBRecord[] records) {
		final StringBuilder query = new StringBuilder(256);
		query.append("INSERT INTO ").append(tableName);
		final int numFields = (fieldNames == null) ? records[0].getNumFields()
				: fieldNames.length;
	...
	public DBInsertOperation makeOperation(
			@Nonnull final DBRecord[] shrinkedRecords) {
		final DBInsertOperation ops = new DBInsertOperation(driverClassName,
				connectUrl, createTableDDL, tableName, fieldNames,
				shrinkedRecords);

...
import gridool.dht.btree.BasicIndexQuery;
...
public abstract class DhtReduceTask extends GridTaskAdapter {
    ...
    private static final class ReduceHandler implements FlushableBTreeCallback {
        ...
        private final DhtReduceTask reducer;
        ...
        public boolean indexInfo(Value key, byte[] value) {
            ...
            if(prevKey != null) {
                ...
                doNext = reducer.process(prevKey.getData(), values);
                if((++counter) == 10) {
                    reducer.reportProgress(-1f);
                }
                this.values = new ArrayList<byte[]>(4);
            }
            this.prevKey = key;
            ...
            if(!reducer.filter(keyData, value)) {
                values.add(value);
            }
            return doNext;
        }
        ...
        public void flush() {
            if(!values.isEmpty()) {
                reducer.process(prevKey.getData(), values);
            }
        }
    }

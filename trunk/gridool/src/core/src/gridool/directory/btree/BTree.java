 ...
 * limitations under the License.
 */
package gridool.directory.btree;
...
public class BTree extends Paged {
    ...
    static {
        DEFAULT_IN_MEMORY_NODES = Primitives.parseInt(Settings.get("gridool.storage.index.btree.nodecache_size"), 4096); // 16m
        BTREE_NODECACHE_PURGE_UNIT = Primitives.parseInt(Settings.get("gridool.storage.index.bfile.nodecache_purgeunit"), 8); // 32k
    }
    ...
    public void init(boolean bulkload) throws IndexException {
        if(!exists()) {
    ...
    @Override
    public boolean open() throws IndexException {
        if(super.open()) {
    ...
    @Override
    public boolean create(boolean close) throws IndexException {
     ...
     */
    public synchronized long addValue(Value value, long pointer) throws IndexException {
        try {
     ...
     */
    public synchronized long removeValue(Value value) throws IndexException {
        try {
    ...
    public synchronized long[] removeValue(Value value, long pointer) throws IndexException {
        try {
     ...
     * @return The pointer that was associated with it
     */
    public synchronized long findValue(Value value) throws IndexException {
     ...
     */
    public synchronized void search(IndexQuery query, CallbackHandler callback) throws IndexException {
        if(query == null) {
     ...
     */
    protected final BTreeNode getRootNode(BTreeRootInfo root) throws IndexException {
        ...
        synchronized(_cache) {
            ...
            if(node == null) {
                ...
                try {
                    ...
                    throw new IndexException("failed to read page#" + page, e);
                }
                if(LOG.isDebugEnabled()) {
    ...
    private final BTreeNode getBTreeNode(BTreeRootInfo root, long page) throws IndexException {
        BTreeNode node;
        synchronized(_cache) {
            node = _cache.get(page);
            if(node == null) {
    ...
    private final class BTreeNode implements Comparable<BTreeNode> {
        ...
        long addValue(Value value, long pointer) throws IOException, IndexException {
            if(value == null) {
        ...
        long removeValue(Value searchKey) throws IOException, IndexException {
        ...
        @Deprecated
        long[] removeValue(Value searchKey, long pointer) throws IOException, IndexException {
            int leftIdx = searchLeftmostKey(keys, searchKey, keys.length);
         ...
         */
        private BTreeNode getChildNode(final int idx) throws IndexException {
         ...
         * Internal to the BTreeNode method
         */
        private void split() throws IOException, IndexException {
        ...
        private void setAsParent() throws IndexException {
         ...
         */
        private void read() throws IOException, IndexException {
            if(!this.loaded) {
        ...
        private void write() throws IOException, IndexException {
            if(!dirty) {
                return;
            }
        ...
        long findValue(Value serarchKey) throws IndexException {
            if(serarchKey == null) {
                throw new IndexException("Can't search on null Value");
            }
        ...
        BTreeNode getLeafNode(SearchType searchType, Value key) throws IOException, IndexException {

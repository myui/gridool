...
import gridool.util.jdbc.handlers.ScalarHandler;
...
public final class ReplicationManager {
    ...
    public ReplicationManager(@Nonnull GridKernel kernel, @Nonnull DBAccessor dba, @Nonnull GridRouter router, @Nonnull GridConfiguration config) {
        this.kernel = kernel;
        this.dba = dba;
        this.router = router;
    ...
    public GridJobFuture<Boolean> replicateTask(@Nonnull GridTask task, @Nonnull GridNodeInfo masterNode) {
        ...
        GridNode trgNode = router.resolve(masterNode);
        final List<GridNode> replicas = trgNode.getReplicas();
        if(replicas.isEmpty()) {
    ...
    public boolean coordinateReplica(@Nonnull GridNode node, int numReplicas, @Nonnull CoordinateReplicaJobConf jobConf) {
        List<GridNode> replicas = replicaSelector.selectReplica(router, node, numReplicas);
        configureReplica(node, replicas, jobConf);
        return replicas.size() == numReplicas;
    }

    public void configureReplica(@Nonnull GridNode node, @Nonnull List<GridNode> replicas, @Nonnull CoordinateReplicaJobConf jobConf) {
        replicaCoordinator.configureReplica(node, replicas, jobConf);
    }
}

...
public final class SQLTranslator {
    ...
    public static int indexOfFirstSelectQuery(final QueryString[] queries) {
        for(int i = 0; i < queries.length; i++) {
            QueryString qs = queries[i];
            if(qs.isSelect()) {
                return i;
            }
        }
        throw new IllegalStateException("Select query is not found: " + Arrays.toString(queries));
    }
